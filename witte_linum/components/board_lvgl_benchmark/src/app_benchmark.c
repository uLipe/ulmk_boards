...
void app_benchmark_run(void)
{
	...
	/*
	 * LV_MEM_ADR points into SDRAM after the dual FBs.  Identity-map the
	 * whole SDRAM window before lv_init() builds the tlsf pool there.
	 */
	if (display_fb_base() == NULL) {
		board_console_puts("lvgl: SDRAM map failed\r\n");
		ulmk_thread_exit();
	}
#if LV_MEM_ADR != 0
