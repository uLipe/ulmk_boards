 ...
 *
 * BSP overlay of upstream LVGL demos/benchmark/lv_demo_benchmark.c
 * (opa card cap for 1024×600 SW, scene logs, shorter widgets scene).
 */
...
static void containers_with_opa_cb(void)
{
    ...
    if(hor_cnt < 1) hor_cnt = 1;
    if(ver_cnt < 1) ver_cnt = 1;
    /* 1024×600 SW: stock 2×3 cards + fall_anim stalls the refr for ages. */
    if(hor_cnt > 2) hor_cnt = 2;
    if(ver_cnt > 2) ver_cnt = 2;
...
static void widgets_demo_cb(void)
{
    ...
    LV_LOG("widgets: creating\r\n");
    lv_demo_widgets();
    LV_LOG("widgets: slideshow\r\n");
    lv_demo_widgets_start_slideshow();
    LV_LOG("widgets: ready\r\n");
}
...
static lv_demo_benchmark_scene_dsc_t scenes[] = {
    ...
    {.name = "Widgets demo",               .scene_time = 8000,           .create_cb = widgets_demo_cb},
...
static void load_scene(uint32_t scene)
{
    ...
    rnd_reset();
    if(scenes[scene].name && scenes[scene].name[0] != '\0') {
        LV_LOG("scene: %s\r\n", scenes[scene].name);
    }
