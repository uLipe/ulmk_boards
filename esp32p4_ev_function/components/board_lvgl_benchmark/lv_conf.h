...
/* clang-format off */
#if 1 /* enabled for board_lvgl_benchmark */
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    /*
     * tlsf pool in PSRAM after dual DPI FBs (board_config ULMK_BOARD_LVGL_*).
     * Numeric literals so lv_conf does not pull board headers into every TU.
     *   FB = 1024*600*2 = 0x12C000; heap @ 0x48000000 + 2*FB + 64K guard
     */
    #define LV_MEM_SIZE   (5U * 1024U * 1024U)
    #define LV_MEM_ADR    0x48268000U
 ...
 * Cap so nested opa_layered cannot OOM-spin forever on 1024×600. */
#define LV_DRAW_LAYER_MAX_MEMORY (1024 * 1024)  /**< [bytes]*/
...
/* SW-only on this board — no DMA2D/PPA path wired yet. */
...
/** Enable log module — WARN+ to RTT via lv_log_register_print_cb. */
#define LV_USE_LOG 1
#if LV_USE_LOG
...
#define LV_USE_ASSERT_MALLOC        1   /**< Reports OOM; handler does not halt */
#define LV_USE_ASSERT_STYLE         0   /**< Check if the styles are properly initialized. (Very fast, recommended) */
#define LV_USE_ASSERT_MEM_INTEGRITY 0   /**< Check the integrity of `lv_mem` after critical operations. (Slow) */
#define LV_USE_ASSERT_OBJ           0   /**< Check the object's type and existence (e.g. not deleted). (Slow) */

/** Halt on assert; message already went through LV_LOG → RTT. */
#define LV_ASSERT_HANDLER_INCLUDE "app_benchmark.h"
#define LV_ASSERT_HANDLER app_lvgl_assert();
...
#if LV_USE_SYSMON
    /** CPU% stub — no kernel idle accounting yet (see port_tick.c). */
    #include <stdint.h>
    uint32_t ulmk_lvgl_idle_percent(void);
    #define LV_SYSMON_GET_IDLE ulmk_lvgl_idle_percent
