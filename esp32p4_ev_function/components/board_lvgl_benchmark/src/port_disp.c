...
/*
 * LVGL display port — DIRECT dual-buffer (ESP32-P4 MIPI-DSI / DW_GDMA).
 *
 * Cache contract (Write-Back PSRAM @ 0x48000000):
 *   - Writeback each flush rectangle so DPI DMA sees CPU writes.  The DMA
 *     only reads the FB, so the lines stay valid: invalidating here would
 *     drop the whole frame from L1/L2 and make the next DIRECT sync refetch
 *     it from PSRAM.
 *   - On last flush, also flush previous frame's dirty rects: LVGL DIRECT
 *     synced them into this buffer via CPU memcpy before draw.
 *   - Clean only the dirty rectangles.  Widening them (or writing back the
 *     whole FB) starves the DPI scanout on the PSRAM port and underruns
 *     DW_GDMA, so the rearm display_present() waits on never arrives.
 *   - display_present() waits for the next DW_GDMA rearm (no busy-wait).
 */
#include <stdint.h>
#include <lvgl.h>
#include <display.h>
...
/* Previous frame's dirty rects — cleaned again after the DIRECT sync. */
...
static void clean_fb_area(void *fb, const lv_area_t *a)
{
	...
	board_dcache_clean_area(fb, FB_STRIDE, a->x1, a->y1, w, h,
				DISPLAY_BPP);
}
...
lv_display_t *port_disp_init(void)
{
	...
	lv_memzero(g_fb0, DISPLAY_FB_BYTES);
	lv_memzero(g_fb1, DISPLAY_FB_BYTES);
