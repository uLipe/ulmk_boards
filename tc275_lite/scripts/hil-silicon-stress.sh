#!/usr/bin/env bash
# HIL silicon stress — flash silicon_stress firmware, dump RAM log for PASS.
set -euo pipefail
...
EXPECT="${HIL_STRESS_EXPECT:-SILICON_STRESS: PASS}"
...
if printf '%s' "$DECODED" | grep -qF "SILICON_STRESS: FAIL"; then
	echo "FAIL: silicon stress reported FAIL" >&2
	exit 1
fi
