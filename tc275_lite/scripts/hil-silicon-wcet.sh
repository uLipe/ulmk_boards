#!/usr/bin/env bash
# HIL silicon WCET — flash silicon_wcet firmware, dump RAM log for PASS.
set -euo pipefail
...
EXPECT="${HIL_WCET_EXPECT:-SILICON_WCET: PASS}"
...
"$OCD" -s "${ULMK_AURIX_PREFIX}/share/openocd/scripts" -s "${ROOT}/openocd" \
	-f "${ROOT}/openocd/tc275_lite_hil.cfg" >/tmp/ulmk-ocd-wcet.log 2>&1 &
sleep 3

GDB_OUT=/tmp/ulmk-gdb-wcet.log
...
if printf '%s' "$DECODED" | grep -qF "SILICON_WCET: FAIL"; then
	echo "FAIL: silicon WCET reported FAIL" >&2
	exit 1
fi
