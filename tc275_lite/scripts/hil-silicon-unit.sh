#!/usr/bin/env bash
# HIL silicon unit — flash silicon_unit firmware, dump RAM log for PASS.
set -euo pipefail
...
EXPECT="${HIL_UNIT_EXPECT:-SILICON_UNIT: PASS}"
...
"$OCD" -s "${ULMK_AURIX_PREFIX}/share/openocd/scripts" -s "${ROOT}/openocd" \
	-f "${ROOT}/openocd/tc275_lite_hil.cfg" >/tmp/ulmk-ocd-unit.log 2>&1 &
sleep 3

GDB_OUT=/tmp/ulmk-gdb-unit.log
...
if printf '%s' "$DECODED" | grep -qF "SILICON_UNIT: FAIL"; then
	echo "FAIL: silicon unit reported FAIL" >&2
	exit 1
fi
